...
export class HTTP {
    ...
    validateExportXml<T>(url: string, options: OptionsWithoutMethodType = {}): Promise<T> {
        return this.request<T>(`/validate-export-xml/${url}`, { ...options, method: METHOD.GET });
    }

    exportXml<T>(url: string, options: OptionsWithoutMethodType = {}): Promise<T> {
        return this.requestXml<T>(`/export-xml/${url}`, { ...options, method: METHOD.POST, responseFormat: 'blob' });
    }
